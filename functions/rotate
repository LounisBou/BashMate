######################################################################################
#
#   La fonction "rotate" de nettoyer un dossier des fichiers et sous-dossiers,
...
function rotate(){ 
  ...
  # Backup path.
	...
	# getops ?
	usage="usage: rotate [-t rotate_time] [-tm time_mode] [-rm rotate_mode] [-p rotate_path] [-bp backup_path] [-v]"
  ...
  then
    rotate_action='mv \"{}\" $backup_path/'
  elif [[ $rotate_mode == 'c' ]]
  then  
    rotate_action='cp -R \"{}\" $backup_path/'
	fi
	  	
	# Création de la commande de rotation.
	rotate_cmd="find $rotate_path -name \"*\" -$time_mode +$rotate_time -exec $rotate_action \;"
