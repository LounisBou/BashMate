######################################################################################
#
#   La fonction "send" permet d'envoyer un dossier ou fichier vers une machine distante,
...
function send(){ 
  ...
  # Local path.
	...
	# getops ?
	usage="usage: send [-lp local_path]  [-h remote_host] [-u remote_user] [-p remote_port] [-rp remote_path] [-v] [-ka|--kill-all]"
  ...
  # Local path :
	...
	# Commande SCP. 
	scp="scp -P $remote_port -r -p $local_path $remote_user@$remote_host:$remote_path"
